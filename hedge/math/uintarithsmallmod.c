            ...
            const Zmodulus *modulus)
{
    ...
    {
        invalid_argument("degree must be a power of two and at least two");
    }
#endif
    if (root == 0)
    {
    ...
    return exponentiate_uint_smallmod(
        root, degree >> 1, modulus) == (modulus->value - 1);
}

bool try_primitive_root(uint64_t degree, const Zmodulus *modulus,
...
bool try_minimal_primitive_root(uint64_t degree,
    const Zmodulus *modulus, uint64_t *destination)
{
    uint64_t root;
    ...
    uint64_t generator_sq = multiply_uint_uint_smallmod(root, root, modulus);
    ...
    for (size_t i = 0; i < degree; i++)
    {
        ...
        current_generator = multiply_uint_uint_smallmod(
...
uint64_t exponentiate_uint_smallmod(uint64_t operand, uint64_t exponent,
    const Zmodulus *modulus)
{
#ifdef HEDGE_DEBUG
    if (modulus->value == 0)
    ...
    while (true)
    {
        if (exponent & 1)
        {
            product = multiply_uint_uint_smallmod(power, intermediate, modulus);
            //swap_uint64(&product, &intermediate);
            intermediate = product;
            if (product >= modulus->value) {
                printf("ERROR! multiply_uint_uint_smallmod(%lu, %lu, %lu) = %lu\n",
                    power, intermediate, modulus->value, product);
            }
        }
        exponent >>= 1;
        if (exponent == 0)
        ...
        product = multiply_uint_uint_smallmod(power, power, modulus);
...
void divide_uint_uint_smallmod_inplace(uint64_t *numerator,
    ...
    uint64_t *quotient)
{
    ...
    else
    {
        ...
        uint64_t* x1_alloc = allocate_uint(uint64_count - 2);
        uint64_t *x1 = x1_alloc;
        uint64_t x2[2];
        uint64_t* quot_alloc = allocate_uint(uint64_count);
        uint64_t *quot = quot_alloc;
        uint64_t* rem_alloc = allocate_uint(uint64_count);
        uint64_t *rem = rem_alloc;
        set_uint_uint(numerator + 2, uint64_count - 2, x1);
        set_uint_uint(numerator, 2, x2); // x2 = (num) % 2^128.

        multiply_uint_uint_6(x1, uint64_count - 2, &modulus->const_ratio[0], 2,
            uint64_count, quot); // x1*A.
        multiply_uint_uint64(x1, uint64_count - 2, modulus->const_ratio[2],
            uint64_count - 1, rem); // x1*B
        add_uint_uint_w_carry(rem, uint64_count - 1, x2, 2, 0, uint64_count, rem); // x1*B + x2;

        size_t remainder_uint64_count = get_significant_uint64_count_uint(rem, uint64_count);
        divide_uint_uint_smallmod_inplace(rem, modulus, remainder_uint64_count, quotient);
        add_uint_uint(quotient, quot, uint64_count, quotient);
        *numerator = rem[0];

        hedge_free(x1_alloc);
        hedge_free(quot_alloc);
        hedge_free(rem_alloc);
...
{
    uint32_t n = (uint32_t)(coeff_count);
    uint32_t m32 = mul_safe(n, (uint32_t)(2));
    uint64_t m = (uint64_t)(m32);
    ...
    else
    {
        ...
        bool sign = steps < 0;
        uint32_t pos_steps = (uint32_t)(abs(steps));

        if (pos_steps >= (n >> 1))
        {
            invalid_argument("step count too large");
        }

        pos_steps &= m32 - 1;
        if (sign)
        {
            steps = (int)(n >> 1) - (int)(pos_steps);
        }
        else
        {
            steps = (int)(pos_steps);
